...
class TransferViewController: UIViewController, SelectCityDelegate, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        selectCityTextField.setBottomBorder2()
        pickUpTextField.setBottomBorder2()
        dropTextfield.setBottomBorder2()
        startDateTextField.setBottomBorder2()
        endDateTextField.setBottomBorder2()
        timeTextField.setBottomBorder2()
        
        pickUpTextField.isUserInteractionEnabled = false
    ...
    func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        ...
        }else if (textField == pickUpTextField || textField == dropTextfield){
            isCityTextField = false
            if (textField == pickUpTextField){
                isPickUpTextField = true
            }else{
                isPickUpTextField = false
            }
            if (selectedCityID == 0){
                AppUtility.sharedInstance.showAlertWithoutIcon(title: "Alert", subTitle: "Please select city?")
            }else{
                let vc = self.storyboard?.instantiateViewController(withIdentifier: "CityListViewController") as! CityListViewController
                vc.mDelegate = self
                vc.cityId = selectedCityID
                vc.tripType = tripTypeID
                self.navigationController?.pushViewController(vc, animated: true)
            }
            return false
    ...
    func openDatePicker()  {
        ...
        picker.addTarget(self, action: #selector(TransferViewController.dueDateChanged), for: UIControlEvents.valueChanged)
    }
    ...
    @objc func dueDateChanged(_ sender:UIDatePicker){
        ...
        if isStartDate{
            start_date = dateObj
            let isGreater = start_date.isGreater12(than: yesterday!)
            if isGreater{
                ...
                if isEndDateSelected{
                    if let userDefaultEndDate = UserDefaults.standard.value(forKey: Constant.END_DATE) as? Date{
                        let isUserDefaultEndDateGreater = userDefaultEndDate.isGreater12(than: start_date)
        ...
        if isEndDate{
            end_date = dateObj
            let isEndDateGreater = end_date.isGreater12(than: start_date)
            if isEndDateGreater{
    ...
    func openTimePicker()  {
        ...
        timePicker.addTarget(self, action: #selector(TransferViewController.startTimeDiveChanged), for: UIControlEvents.valueChanged)
    }
    ...
    func sendToSearchCabVC()  {
        ...
        let journeyRoute = "\(selectCityTextField.text!) -> " + "\(pickUpTextField.text!) To \(dropTextfield.text!) | " + "\(dayCount)-day(s) | " + "\(travelDate) | " + "\(pickupTime)"
        ...
        vc.mTravel_type_option = "3"
        vc.source_city_name = selectCityTextField.text!
        vc.pickup_location = pickUpTextField.text!
        vc.drop_location = dropTextfield.text!
        ...
        if selectCityTextField.text != ""{
            if pickUpTextField.text != ""{
                if dropTextfield.text != ""{
                    ...
                    AppUtility.sharedInstance.showAlertWithoutIcon(title: "Alert", subTitle: "Please select drop Location?")
                }
            }else{
                AppUtility.sharedInstance.showAlertWithoutIcon(title: "Alert", subTitle: "Please select pickUp Location?")
            }
        }else{
            AppUtility.sharedInstance.showAlertWithoutIcon(title: "Alert", subTitle: "Please select city?")
        }
    ...
    func getCityNameAndID(_ mName: String, mID: Int) {
        ...
        }else if (isPickUpTextField){
            pickUpTextField.text = mName
        }else{
            dropTextfield.text = mName
        }
    }
}
extension Date {
    func isGreater12(than date: Date) -> Bool {
        return self >= date
    }
    func isSmaller12(than date: Date) -> Bool {
        return self < date
    }
    func isEqual12(to date: Date) -> Bool {
        return self == date
    }
}
